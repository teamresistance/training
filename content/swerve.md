...
The swerve drive subsystem is implemented using the AdvantageKit abstraction, relying on the`SwerveDriveIO`interface. It integrates hardware control, odometry, and pathfinding setup. Refer to[Subsystem IO Abstraction](abstract-io.html)if you forgot how it works!
...
- Vision Integration: The methods`addVisionMeasurement()`and`addAutoVisionMeasurement()`allow external vision systems (like Limelight or PhotonVision) to correct the estimated pose, improving accuracy by miles - relying on odometry alone is bound to fail as the wheels can slip frequently.[See this](https://v6.docs.ctr-electronics.com/en/latest/docs/application-notes/update-frequency-impact.html)for more unneccessary - but cool to know - odometry info
